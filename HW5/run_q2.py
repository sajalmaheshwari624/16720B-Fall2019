...
import copy
batches = get_random_batches(x,y,5)
xSample, ySample = batches[0]
h1 = forward(xSample,params,'layer1')
probs = forward(h1,params,'output',softmax)
delta1 = probs - ySample
delta2 = backwards(delta1,params,'output',linear_deriv)
backwards(delta2,params,'layer1',sigmoid_deriv)
...
for k,v in params.items():
    if k == 'Wlayer1' or k == 'Woutput': 
        for s1 in range(v.shape[0]) :
            for s2 in range(v.shape[1]) :
                param_neg = copy.deepcopy(params_orig)
                param_neg[k][s1][s2] -= eps

                h1_neg = forward(xSample,param_neg,'layer1')
                probs_neg = forward(h1_neg,param_neg,'output',softmax)
                loss_neg, _ = compute_loss_and_acc(ySample, probs_neg)

                param_pos = copy.deepcopy(params_orig)
                param_pos[k][s1][s2] += eps

                h1_pos = forward(xSample,param_pos,'layer1')
                probs_pos = forward(h1_pos,param_pos,'output',softmax)
                loss_pos, _ = compute_loss_and_acc(ySample, probs_pos)
                #print ("Weight negative", loss_pos)
                params['grad_' + k][s1][s2] = (loss_pos - loss_neg) / (2*eps) 
    ...
    if k == 'blayer1' or k == 'boutput':
        for s in range(v.size) :
            ...
            probs_neg = forward(h1_neg,param_neg,'output',softmax)
            loss_neg, _ = compute_loss_and_acc(ySample, probs_neg)
            #print ("Bias negative", loss_neg)
            param_pos = copy.deepcopy(params_orig)
            param_pos[k][s] += eps
            h1_pos = forward(xSample,param_pos,'layer1')
            probs_pos = forward(h1_pos,param_pos,'output',softmax)
            loss_pos, _ = compute_loss_and_acc(ySample, probs_pos)
            #print ("Bias positive", loss_pos)
            params['grad_' + k][s] = (loss_pos - loss_neg) / (2*eps)
