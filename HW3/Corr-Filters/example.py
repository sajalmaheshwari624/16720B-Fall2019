...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        lamda = 0
        Xtr = np.transpose(X)
        gn = np.matmul(np.linalg.inv((np.matmul(X, Xtr) + np.eye(X.shape[0])*lamda)),np.matmul(X,Y))
        gn = np.reshape(gn, (29,45))
        filterOut = gn
        #print (filterOut)
        fig,ax = plt.subplots(1)
        ax.imshow(filterOut)
        imName = str('im_0_filter.png')
        plt.savefig(imName)
        plt.close()
        out1Corr = scipy.ndimage.correlate(img, gn)
        fig, ax = plt.subplots(1)
        ax.imshow(out1Corr)
        plt.savefig('im_0_correlate.png')
        plt.close()
        out1Conv = scipy.ndimage.convolve(img, gn)
        fig, ax = plt.subplots(1)
        ax.imshow(out1Conv)
        plt.savefig('im_0_convolve.png')
        plt.close()
        ...
        fig,ax = plt.subplots(1)
        ax.imshow(filterOut)
        imName = str('im_1_filter.png')
        plt.savefig(imName)
        plt.close()
        out1Corr = scipy.ndimage.correlate(img, gn)
        fig, ax = plt.subplots(1)
        ax.imshow(out1Corr)
        plt.savefig('im_1_correlate.png')
        plt.close()
        out1Conv = scipy.ndimage.convolve(img, gn)
        fig, ax = plt.subplots(1)
        ax.imshow(out1Conv)
        plt.savefig('im_1_convolve.png')
        plt.close()
